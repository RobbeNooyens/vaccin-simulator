...
bool Planning::properlyInitialized() const {
    return initCheck == this;
}

std::vector<std::vector<std::pair<int,std::map<Vaccine*, unsigned int> > > > Planning::getPlanned() const {
    REQUIRE(properlyInitialized(), "Simulator object hasn't been initialized properly!");
    return planned;
}
...
bool Planning::is_valid(unsigned int day, int vaccine_idx, int center_idx, std::vector<Vaccine*>& vaccins, std::vector<VaccinationCenter*>& centers, std::vector<Hub*>& hubs, unsigned int cycles) {
    REQUIRE(properlyInitialized(), "Simulator object hasn't been initialized properly!");
    if (((day+vaccins[vaccine_idx]->getRenewing() < cycles) && (planned[center_idx][day+vaccins[vaccine_idx]->getRenewing()].first == (int) centers[center_idx]->getCapacity()))) {
        return false;
    }
    ...
    unsigned int count = cycles/(hubs[center_idx]->getVaccines()[day]->getRenewing()+1);
...
void Planning::generatePlanning(std::vector<Hub*>& hubs, std::vector<VaccinationCenter*>& centers, std::vector<Vaccine*>& vaccins, unsigned int cycles) {
    ...
    for (int center_idx = 0; center_idx < (int) centers.size(); center_idx++) {
        ...
        for (unsigned int day = 0; day < cycles; day++) {
            ...
            for (int vaccine_idx = 0; vaccine_idx < (int) vaccins.size(); vaccine_idx++) {
                ...
                while ((TotalInhabitantsInCenter != 0) && (planned[center_idx][day].first < (int) centers[center_idx]->getCapacity()) && is_valid(day,vaccine_idx,center_idx, vaccins, centers, hubs, cycles)) {
